...
## LLEGEIX AIXO PRIMER: hi ha documents a part
...
| Fitxer | Què hi ha |
|---|---|
| `Motor.ps1` | rutes, config, càrrega de mòduls, `%LOCALAPPDATA%`, instància única, Word, accés al catàleg |
...
## On va cada cosa nova

Abans d'escriure codi nou, mira si ja hi ha el lloc. Si no hi és, **fes-lo
genèric un cop** i fes-lo servir també als llocs que ja existien; un helper que
només fa servir el codi nou no simplifica res.
...
**Els límits que vigila la suite:** cap fitxer de `suport/` passa de **1.200
línies** (quatre excepcions amb sostre propi a `06-guards.ps1`) i cap nom de
funció es defineix a **dos fitxers**. Si un fitxer toca el límit, parteix-lo per
responsabilitats (com `LlicenciaDades` / `Blocs` / `Pantalles`); **no apugis el
límit**. El mapa i les propostes pendents són a
`suport/documentacio/arquitectura.md`.
...
4. Les amplades de columna anaven desplaçades una posició (vegeu *Seguiment* a `documentacio/eines.md`).
...
a la secció de *Recordatoris periòdics* de `documentacio/eines.md`.
